...
LPUTSTR COCSInventoryFrontApp::getInstallFolder()
{
	return m_csInstallFolder;
}

LPUTSTR COCSInventoryFrontApp::getDataFolder()
{
...
BOOL COCSInventoryFrontApp::setDataFolder( LPUTSTR lpstrDir)
{
...
LPUTSTR getInstallFolder()
{
	return theApp.getInstallFolder();
}

LPUTSTR getDataFolder()
{
...
CString LookupError( DWORD64 dwErr)
{
	LPVOID lpMsgBuf;
	DWORD64 dwReturn;
...
//		hModule = LoadLibraryEx(TEXT("netmsg.dll"), NULL, LOAD_LIBRARY_AS_DATAFILE);
	...
	{
		csMsg.Format( _T( "Error code %lu = %s"), dwErr, (LPUTSTR)lpMsgBuf);
		// Free the buffer.
		LocalFree( lpMsgBuf );
	}
...
BOOL isRequired( LPUTSTR lpstrCommandLine, CString csOption)
{
...
CString getParamValue( LPUTSTR lpstrCommandLine, CString csParam)
{
	CString	csCommand = lpstrCommandLine,
			csOptionName,
			csValue,
...
BOOL setDataFolder( LPUTSTR lpstrDir)
{
	// normal function body here
