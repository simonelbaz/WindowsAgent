...
LPUTSTR OCSINVENTORYFRONT_API getInstallFolder();
// Get application data storage folder (All Users)
LPUTSTR OCSINVENTORYFRONT_API getDataFolder();
...
CString OCSINVENTORYFRONT_API LookupError( DWORD64 dwErr);
...
BOOL OCSINVENTORYFRONT_API isRequired( LPUTSTR lpstrCommandLine, CString csOption);
// Retrieve value from a command line option ("/XML:path_to_file.xml")
CString OCSINVENTORYFRONT_API getParamValue( LPUTSTR lpstrCommandLine, CString csParam);

// Get application data storage folder (All Users)
BOOL OCSINVENTORYFRONT_API setDataFolder( LPUTSTR lpstrDir);
