...
public:
	...
	BOOL Open( LPUTSTR lpstrFile, BOOL bForWriting = FALSE, BOOL bExclusive = FALSE);
	...
	BOOL AddPackage( LPUTSTR lpstrPackageID);
	...
	static int CleanDuplicates( LPUTSTR lpstrFile, BOOL bForce = FALSE);
	// Add package to an history file (will be opened and then closed)
	static BOOL AddPackage( LPUTSTR lpstrFile, LPUTSTR lpstrPackageID);
