...
public: // Methods
	...
	void log(int, LPUTSTR, ...);
	// Set the current log priority
	void setLogLevel(DWORD64);
	...
	void setApplication( LPUTSTR lpstrApp = NULL);
...
private: // Methods
	...
	BOOL isOpen();
	// "Normal" log
	void log_notice( LPUTSTR lpstrFormat, va_list args);
	// Warning log, but can continue
	void log_warning( LPUTSTR lpstrFormat, va_list args);
	// Blocking error log
	void log_error( LPUTSTR lpstrFormat, va_list args);
	// Debug log
	void log_debug( LPUTSTR lpstrFormat, va_list args);
	// Trace log for full mode debug
	void log_trace( LPUTSTR lpstrFormat, va_list args);
