...
public: // Methods
	...
	LPUTSTR getCommunicationProvider();
	...
	void setXmlFolder( LPUTSTR lpstrFolder = NULL);
	...
	void setLocalInventory( LPUTSTR lpstrFolder = NULL);
	// Does we disable software inventory (/NOSOFTWARE)
	...
	void setTagText( LPUTSTR lpstrText = NULL);
	...
	void setIpDiscoverRequired(LPUTSTR lpstrNetwork = NULL);
	// Set if IpDiscover latency is specified
	void setIpDiscoverLatency(LPUTSTR lpstrLatency = NULL);
	// Does agent launches a fast IpDiscover scan, without taking care of network load, or not
	...
	void setCommunicationProvider( LPUTSTR lpstrDll);

	// Write Administrative info to admininfo.conf hidden file in agent install folder
	BOOL writeAccountInfos( LPUTSTR lpstrName, LPUTSTR lpstrValue);

private: //Methods
	void setConfigFile( LPUTSTR lpstrFile = NULL);
