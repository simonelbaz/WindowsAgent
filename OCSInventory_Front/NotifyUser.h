...
public:
	...
	UINT AskQuestion( LPUTSTR lpstrText);

	// Display an information to userusing MessageBox
	// Return must be 0 = OK, otherwise error
	UINT ShowInformation( LPUTSTR lpstrText);

	// Display pre-installation status message
	// Return must be 0 = OK, 1 = Cancel, 2 = Delay, otherwise error
	UINT ShowPreInstall( LPUTSTR lpstrText, BOOL bCancelAllowed = FALSE, BOOL bDelayAllowed = FALSE, UINT uTimeOut = 0);

	// Display post-installation status message
	// Return must be 0 = OK, 2 = Delay, otherwise error
	UINT ShowPostInstall( LPUTSTR lpstrText, BOOL bRebootRequired = FALSE, BOOL bDelayAllowed = FALSE, UINT uTimeOut = 0);

	// Ask TAG to user and store result into csTag
	// Return must be 0 = OK, otherwise error
	UINT AskTag( LPUTSTR lpstrText, CString &csTag);
